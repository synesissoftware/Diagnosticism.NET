# Diagnosticism.NET - Contributing <!-- omit in toc -->
...
requests are welcome on [GitHub](https://github.com/synesissoftware/Diagnosticism.NET).


## Reporting issues

Use [GitHub Issues](https://github.com/synesissoftware/Diagnosticism.NET/issues). Include:

* **Diagnosticism.NET** version (NuGet package version or release tag);
...
3. Build and test:
   ...
   ```bash
   dotnet build Diagnosticism.NET.sln
   dotnet test Diagnosticism.NET.sln
   ```
...
* Public API in **`src/Diagnosticism.NET/`** must have XML documentation (`///`);
...
3. **Capitalisation**: the first word of a comment should be capitalised,
   unless it is a well-known name (for example, `Diagnosticism`, `NuGet`);
...
* XML documentation on affected public members in **`src/Diagnosticism.NET/`**;
